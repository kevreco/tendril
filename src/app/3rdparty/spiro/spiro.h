#ifndef SPIRO_SPIRO_H
#define SPIRO_SPIRO_H

/*
ppedit - A pattern plate editor for Spiro splines.
Copyright (C) 2007 Raph Levien Licensed under the Apache License,
Version 2.0 <LICENSE-APACHE or
...
#ifdef SPIRO_IMPLEMENTATION

#define SPIRO_ENABLE_IS_FINITE_CHECK 1
...
#ifdef SPIRO_ENABLE_IS_FINITE_CHECK
        if (!is_finite(s, nseg))
        {
            break;
        }
                ...
                bezctx* bc, int depth)
{
    ...
    if (!(bend > 1e-8)) {
        ...
        rot = seg_th - th;
        if (depth > 5 || bend < 1.) {
            th_even = (1. / 384.0) * ks[3] + (1. / 8.0) * ks[1] + rot;
            th_odd = (1. / 48.0) * ks[2] + .5 * ks[0];
            ...
            ksub[0] = .5 * ks[0] - .125 * ks[1] + (1. / 64.0) * ks[2] - (1. / 768.0) * ks[3];
            ksub[1] = .25 * ks[1] - (1. / 16.0) * ks[2] + (1. / 128.0) * ks[3];
            ksub[2] = .125 * ks[2] - (1. / 32.0) * ks[3];
            ksub[3] = (1. / 16.0) * ks[3];
            thsub = rot - .25 * ks[0] + (1. / 32.0) * ks[1] - (1. / 384.0) * ks[2] + (1. / 6144.0) * ks[3];
